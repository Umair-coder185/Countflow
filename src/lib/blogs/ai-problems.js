const aiproblems = `
<article>
  <p>You're watching ChatGPT write out a good answer, and it just... stops. Mid-word, mid-thought, no error message. Or Claude tells you your message is too long when your prompt looked short. Or the response you were reading a second ago vanishes and reappears as "network error."</p>
  
  <p>These look like the same bug. They're actually three unrelated problems, and each one has a completely different fix.</p>

  <div>
    <h3>Quick Fix: Why Did AI Stop Writing?</h3>
    <p>If ChatGPT or Claude stops generating text mid-sentence with no error message, it has hit its <strong>Max Output Token limit</strong>.</p>
    <ul>
      <li><strong>The Fix:</strong> Simply type <em>"Continue exactly where you left off"</em> and hit send. The AI will resume generating the rest of the text.</li>
      <li><strong>The Prevention:</strong> Check your prompt length in our <a href="/tools/ai-token-counter">Free AI Token Counter</a> before you send it to avoid hitting the hidden ceiling.</li>
    </ul>
  </div>
  ...
  <figure>
    <img src="/blogs/ai-max-output-token-scale.png" alt="ai max output scale problems"/>
  </figure>
  ...
  <ul>
    ...
    <li>If you're building on the API, raise your <code>max_tokens</code> / <code>max_output_tokens</code> parameter, since the default is often far lower than the model actually supports.</li>
  </ul>
